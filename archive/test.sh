...
function gentooinstall_phase_bootloader
{
	...
	case "$BOOTLOADER" in
		...
		"bios-lilo")
			...
			;;
		"uefi-grub")
			gentooinstall_phase_bootloader_uefi_grub || return
			;;
		"uefi-refind")
...
function gentooinstall_phase_bootloader_uefi_grub
{
	gentooinstall_phase_procsysdev
	local PREFIX="$CHROOTARCH chroot $DESTINATION_ROOT"
	if [ "$DESTINATION" = '/' ]; then
	...
	gentooinstall_quiet $PREFIX grub-install --target=x86_64-efi --efi-directory=/boot --removable || return
	gentooinstall_quiet $PREFIX grub-mkconfig -o /boot/grub/grub.cfg || return
}
