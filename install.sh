...
export CFG_BLOCK_DEVICE="$(prompt_value "Nom du périphérique cible (EX : /dev/${DISK})" "$CFG_BLOCK_DEVICE")"
export CFG_PART_PREFIX="$(prompt_value "Préfixe de la partition (EX : 'p' pour les disques NVMe, rien pour les HDD/SSD)" "$CFG_PART_PREFIX")"
export CFG_BLOCK_PART="${CFG_BLOCK_DEVICE}${CFG_PART_PREFIX}"
export CFG_PART_UEFI="$(prompt_value "Voulez-vous utiliser le mode UEFI ?" "$CFG_PART_UEFI")"
export CFG_PART_BOOT_SIZE="$(prompt_value "Taille de la partition boot (en Mo, par défaut : 256)" "$CFG_PART_BOOT_SIZE")"
export CFG_PART_SWAP_SIZE="$(prompt_value "Taille de la partition swap (en Mo, par défaut : 4096)" "$CFG_PART_SWAP_SIZE")"
export CFG_PART_ROOT_SIZE="$(prompt_value "Taille de la partition root (en %, par défaut  : 100)" "$CFG_PART_ROOT_SIZE")"
export CFG_MUSL="$(prompt_value "Utiliser MUSL au lieu de la bibliothèque C GNU ?" "$CFG_MUSL")"
export CFG_LLVM="$(prompt_value "Utiliser LLVM au lieu de GCC ?" "$CFG_LLVM")"
export CFG_TIMEZONE="$(prompt_value "Fuseau horaire du système (par défaut : Europe/Paris)" "$CFG_TIMEZONE")"
export CFG_LOCALE="$(prompt_value "Locale du système (par défaut : fr_FR)" "$CFG_LOCALE")"
export CFG_HOSTNAME="$(prompt_value "Nom d'hôte du système (par défaut : gentoo)" "$CFG_HOSTNAME")"
export CFG_NETWORK_INTERFACE="$(prompt_value "Nom de l'interface réseau (par défaut : enp0s3)" "$CFG_NETWORK_INTERFACE")"
export CFG_KEYMAP="$(prompt_value "Disposition du clavier à utiliser" "$CFG_KEYMAP")"
