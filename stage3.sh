...
if [[ "${CFG_PART_UEFI}" == "y" ]]; then
  ...
  log_msg INFO "mount boot partition (MBR)" >> /var/log/installer.log
...
if [[ "${CFG_MUSL}" == "y" ]]; then
  log_msg INFO "configure timezone (musl)" >> /var/log/installer.log
  emerge --ask n --config sys-libs/timezone-data
  echo "TZ=\"/usr/share/zoneinfo/${CFG_TIMEZONE}\"" >> /etc/env.d/00musl

  log_msg INFO "configure locales (musl)" >> /var/log/installer.log
...
if [[ "${CFG_PART_UEFI}" == "y" ]]; then
  ...
  log_msg INFO "set /boot in fstab" >> /var/log/installer.log
...
if [[ "${CFG_PART_UEFI}" == "y" ]]; then
  ...
  log_msg INFO "install grub2" >> /var/log/installer.log
  emerge --ask n sys-boot/grub

  log_msg INFO "install MBR bootloader" >> /var/log/installer.log
