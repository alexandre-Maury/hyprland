...
log_msg INFO "Mise à jour de l'ensemble @world (@system et @selected)"
...
log_msg INFO "Définition du mot de passe pour l'utilisateur ${CFG_USER}" 
echo "${CFG_USER}:${CFG_USER_PASSWORD}" | chpasswd
log_msg INFO "Ajout de ${CFG_USER} au fichier sudoers"
echo "${CFG_USER} ALL=(ALL) ALL" > /etc/sudoers.d/${CFG_USER}
chmod 440 /etc/sudoers.d/${CFG_USER}

log_msg INFO "Configuration de la carte clavier"
sed -i '/^keymap/s/=.*$/=$"'"${CFG_KEYMAP}"'"/' /etc/conf.d/keymaps
rc-update add keymaps boot
rc-service keymaps restart

log_msg INFO "Installation de syslog" >> 
emerge --ask n app-admin/sysklogd
rc-update add sysklogd default

log_msg INFO "Installation de crond"
emerge --ask n sys-process/cronie
rc-update add cronie default

log_msg INFO "Installation de l'indexeur de fichiers" 
emerge --ask n sys-apps/mlocate

log_msg INFO "Installation des outils du système de fichiers" 
...
if [[ "${CFG_PART_UEFI}" == "y" ]]; then
  log_msg INFO "Installation de grub2 avec EFI"
  echo "GRUB_PLATFORMS=\"efi-64\"" >> /etc/portage/make.conf
  emerge --ask n sys-boot/grub

  log_msg INFO "Installation du chargeur de démarrage EFI"
  grub-install --target=x86_64-efi --efi-directory=/boot/efi
else
  log_msg INFO "Installation de grub2"
  emerge --ask n sys-boot/grub

  log_msg INFO "Installation du chargeur de démarrage MBR"
  grub-install ${CFG_BLOCK_PART}
fi

log_msg INFO "Configuration du chargeur d'amorçage"
